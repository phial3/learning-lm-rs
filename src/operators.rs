...
pub fn rms_norm(y: &mut Tensor<f32>, x: &Tensor<f32>, w: &Tensor<f32>, epsilon: f32) {
    let _ndim = x.shape().len();
    let ndim_y = y.shape().len();
    let _ndim_w = w.shape().len();

    // 检查维度是否匹配
    // assert!(x.shape()[ndim-1]==y.shape()[ndim_y-1]);
    // assert!(x.shape()[ndim-2]==w.shape()[ndim_w-1]);
    let seq_len = y.shape()[ndim_y - 2]; //2行
    let total_seq_len = y.shape()[ndim_y - 1]; //3列

    // let batch = y.size() / seq_len ;
    let _x = x.data(); // 获取输入 x 的数据
    let _w = w.data(); // 获取权重 w 的数据
