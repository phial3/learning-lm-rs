...
pub fn rms_norm(y: &mut Tensor<f32>, x: &Tensor<f32>, w: &Tensor<f32>, epsilon: f32) {
    //传参三个张量：输入、输出、权重张量 epsilon：一个很小的值，用于避免除零错误，常见的做法是在计算 RMS 时加上一个微小的常数来稳定计算。
    // todo!("实现 rms_norm，计算前做一些必要的检查会帮助你后续调试");//均方根计算逻辑：对当前行的元素平方求和、除以元素个数，再加上 epsilon 后取平方根
    assert!(
        w.size() == x.shape().last().copied().unwrap_or(0),
        "w向量的分量个数和x向量的分量个数不一样"
    ); //因为w是一维的，所以size返回的才是向量的分量个数
    assert!(w.shape().len() == 1, "w不是一维向量");
    assert!(
        x.shape().last().copied().unwrap_or(0) == y.shape().last().copied().unwrap_or(0),
        "x，y向量分量长度一样"
    );
    ...
    unsafe {
        y.data_mut()
            .par_chunks_mut(vec_num)
            .zip(x_data.par_chunks(vec_num)) //按照分量个数划分向量获取所有的向量-zip将xy合成一个元组后面遍历的时候方便操作，zip是在原内存空间上操作的
            .for_each(|(y_slice, x_slice)| {
                //y_slice和x_slice这个代表张量xy的每一个长度为n的向量
                let rms = (x_slice.iter().map(|&val| val * val).sum::<f32>() / vec_num as f32
                    + epsilon)
                    .sqrt(); //分母部分
